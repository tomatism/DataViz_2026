...
lapply(c("tidyverse", "ggplot2", "lubridate", "patchwork", "geofacet", "ggrepel", "GGally", "corrgram", "ggcorrplot", "broom", "marginaleffects", "scales"),  pkgTest)
...
temp_plot <- ggplot(IRE_weather[IRE_weather$station=="Dublin",], aes(x = date, y = maxtp)) +
  geom_line() +
  geom_smooth() +
  scale_y_continuous(sec.axis = sec_axis(transform = ~ (. * 9/5) + 32, name = "Fahrenheit")) +
  labs(x = "Date", y = "Celsius") +
  theme_minimal()
...
jud_rew <- judicial_review |>
  select(CASE, DATE, YEAR, EFFECT, `S.DATE`,
         `TIME.TO.DECISION`, CONGRESS, AREA1, REVERSD,
         DISSENT, `ATT.GEN`) |>
  filter(YEAR >= 1900)
...
jud_rew_const_area <- jud_rew |>
  group_by(AREA1) |>
  summarise(count = n()) |>
  arrange(desc(count))
print(jud_rew_const_area)
...
jud_rew <- jud_rew |>
  mutate(
    date_diff = as.Date(DATE, format = "%d/%m/%Y") -
      as.Date(S.DATE, format = "%d/%m/%Y"),
    DATE = dmy(DATE),
    S.DATE = dmy(S.DATE)
  )

average_time <- jud_rew |>
  summarise(mean_date_diff = mean(as.numeric(date_diff), na.rm = TRUE))


# What is the most dissenting votes that were cast in the case in which the USSC reversed the decision of the lower court?

jud_rew_votes <- jud_rew |>
  filter(REVERSD == 1) |>
  arrange(desc(DISSENT))
...
p2 <- judicial_review |>
  ...
  labs(
    x = "Year",
    y = "Number of cases",
    title = "Not Upheld Judicial Cases by Year"
  ) +
  theme_bw()
...
ggplot(data = area_time, aes(x = YEAR, y = n)) +
  geom_line() + 
  facet_wrap(~AREA1) +
  geom_smooth()+
  labs(
...
model_jr <- lm( data = jud_rew,
  TIME.TO.DECISION ~ EFFECT + DISSENT + CONGRESS + ATT.GEN
)
# Deliverables:
